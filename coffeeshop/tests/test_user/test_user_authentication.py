...
def test_get_user_exists_integration(backend: EmailAuthBackend, user_with_email: User) -> None:
    ...
    assert backend.get_user(user_with_email.id) == user_with_email
...
@pytest.mark.parametrize(
    "user_id,expected",
    [
        (1, True),  # existing user (id=1)
        (999, False),  # non-existent user
        (None, False),  # None instead of ID
        ("abc", False),  # string instead of number
    ],
)
def test_get_user_parameters(
    backend: EmailAuthBackend, user_with_email: User, user_id: int | str | None, expected: bool
) -> None:
    """Test user retrieval with various ID types."""
